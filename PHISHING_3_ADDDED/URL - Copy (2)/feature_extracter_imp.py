...
def get_domain(url):
    ...
    print('domain is :',tldextract.extract(url).domain)
...
def extract_features(url, status):
    ...
    if state:
        ...
        #print(words_raw, words_raw_host, words_raw_path,path,subdomain,domain,extracted_domain,hostname, domain, path)
        ...
        print('extract_data_from_URL completed')
        row = [url,
               # url-based features
               ...
               # # # content-based features
                 ctnfe.nb_hyperlinks(Href, Link, Media, Form, CSS, Favicon),#60
                 ...
                 ctnfe.external_favicon(Favicon),#69
               ...
               #  # additional content-based features
                ...
                # # # thirs-party-based features
                 ...
                 #trdfe.web_traffic(url),#------->not______working 
                 trdfe.dns_record(domain),
                 #trdfe.google_index(url),#------->not______working 
                 trdfe.page_rank(key,domain)#86
               # status
               #status
               ]
        #print(row)
...
all_columns =['length_url', 'length_hostname', 'ip', 'nb_dots', 'nb_hyphens', 'nb_at',
       ...
       'dns_record', 'page_rank', 'special', 'is_encoded', 'entropy']

cols_to_scale  = ['length_url', 'length_hostname',  'nb_dots', 'nb_hyphens', 'nb_at',
       'nb_qm', 'nb_and', 'nb_eq', 'nb_underscore', 'nb_slash', 'nb_colon',
       'nb_space', 'nb_www', 'nb_com',  'ratio_digits_url',
       'ratio_digits_host', 'nb_subdomains','nb_redirection','length_words_raw', 'char_repeat', 'shortest_words_raw','shortest_word_host', 'shortest_word_path', 'longest_words_raw',
       'longest_word_host', 'longest_word_path', 'avg_words_raw',
       'avg_word_host', 'avg_word_path', 'statistical_report', 'nb_hyperlinks',
       'ratio_intHyperlinks', 'ratio_extHyperlinks', 'nb_extCSS',
       'ratio_extRedirection', 'ratio_extErrors',
        'links_in_tags', 'ratio_intMedia', 'ratio_extMedia', 'safe_anchor','domain_registration_length', 'domain_age', 'dns_record', 'page_rank','special','entropy']
  
columns_to_drop = ['submit_email',
 'punycode',
 'right_clic',
 'nb_dollar',
 'path_extension',
...
import time
import joblib
loaded_scaler = joblib.load('3_features.joblib')
import _3_features
def fetch88(url):
    program_output = extract_features(url,'200')
    if program_output==None:
        return []
    program_output  =program_output+_3_features.fetch(url)#added
    print(program_output)
    program_output_numpy= np.array([program_output[1:]])
    program_output_DF= pd.DataFrame(program_output_numpy,columns =all_columns)
    print(program_output_DF)
    program_output_DF = program_output_DF.drop(columns = columns_to_drop,axis =1)
    program_output_DF = program_output_DF.astype(float)
    program_output_DF_to_scale = program_output_DF[cols_to_scale]
    scaled_input = loaded_scaler.transform(program_output_DF_to_scale)
    program_output_DF [cols_to_scale] = scaled_input
    return program_output_DF.to_numpy()[0]
#print(fetch88('https://www.answers.com/Q/PnP_is_the_acronym_for_what_term'))